...
export const addDeckDetails = async (deck) => {
  try {
...
export const resetDecks = async () => {
  try {
    await AsyncStorage.setItem(DECKS_STORAGE_KEY, JSON.stringify((recieveDecks())));
  } catch (err) {
    console.log('Error while resetting decks :: ',err);
  }
}
